...
void Rock::draw(int camera_x, int camera_y) {
    if (!hit_animation) {
        ...
        // ALLEGRO_SAMPLE* music11 = al_load_sample("resources/rock.wav");
        // std::cout << al_play_sample(music11, 1.0, 0.0, 1.0, ALLEGRO_PLAYMODE_ONCE, 0); //(SAMPLE NAME, gain(volumn), pan(balance), speed, play_mode, sample_id)
