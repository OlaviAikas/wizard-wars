...
WaterShield::WaterShield(int start_x, int start_y, float dir_x, float dir_y,bool subshield) 
            : Shield::Shield(start_x, start_y, dir_x, dir_y, 16, 16, false) {
    damage = 1;
    sprite = al_load_bitmap("resources/shield1.bmp");
    int time1 = 0;
...
void WaterShield::draw(int camera_x, int camera_y) {
...
void WaterShield::on_collision(MapObject &other) {
...
std::list<std::list<int>> WaterShield::generate_subshield_coordinates() {
    float angle = atan2(dir_y,dir_x);
